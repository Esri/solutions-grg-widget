GRG Widget: Unit testing
===========================================

The GRG Widget uses [Intern](http://theintern.io/) as its test runner.  Unit tests can be
...
4. Deploy the GRG widget to the test WAB app widgets folder 
5. Install Intern components to the root folder to a web server. Note: NPM needs to be installed before calling
```
npm install intern
```
6. Open a browser and navigate to http://hostname/path-to-distance-and-direction-folder/node_modules/intern/client.html?config=widgets/GRG/tests/intern
